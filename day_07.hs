...
import qualified Data.MultiMap as Map (fromList, fromMap, lookup, toList)
import qualified Data.Set as Set (empty, insert)
import Text.ParserCombinators.ReadP (eof, many1, option, readP_to_S, satisfy, string)
...
countChildren m =
  let aux k [] = k
      aux k ((n, x) : xs) = aux (k + n) (xs ++ map (Data.Bifunctor.first (n *)) (Map.lookup x m))
